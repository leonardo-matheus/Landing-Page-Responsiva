;(function($){
	...
	// Rolagem suave usando o easing do jQuery
	...
	if (quoteForm.length > 0) {
	  if( !$().validate || !$().ajaxSubmit ) {
		  console.log('quoteForm: jQuery Formulário ou Validação de Formulário não estão definidos.');
		  return true;
	  }
	  // Formulário de Cotação - página inicial
	  if (quoteForm.length > 0) {
	...
	if ($particles_js.length > 0 ) {
		...
		// Final das particulas
      );
	}
	
	// Ativar menu de página quando clicar no link
